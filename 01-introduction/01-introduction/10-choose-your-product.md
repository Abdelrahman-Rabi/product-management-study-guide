---
title: "Exercise #1 Choose Your Product"
...
```mermaid
flowchart TD
    subgraph Select["1. Selection Ground Rules"]
        A["Is it Tech-Powered?"] -->|Yes| B["New or Existing?"]
        A -->|No| Reject["Reject Product"]
        B --> C["Familiar & Passionate?"]
        C -->|Yes| D["User Access Available?"]
        C -->|No| Reject
        D -->|Yes| Evaluate["Evaluate Scale"]
        D -->|No| Reject
    end
    subgraph Scope["2. Scope Calibration"]
        Evaluate --> E{"Is it a Large Scale Platform?"}
        E -->|Yes| F["Constrain to Sub-Product / Specific Feature Role"]
        E -->|No| G["Use Entire Product Scope"]
        F --> H["Final Study Canvas"]
        G --> H
    end
```
